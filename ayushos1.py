...
class SchedulerSimulator:
    ...
    def create_widgets(self):
        ...
        self.alg_combo['values'] = ("FCFS", "SJF", "SJF Preemptive", "Priority", "Round Robin")
    ...
    def add_process(self):
        try:
            ...
            if self.priority_entry['state'] == 'normal':
    ...
    def sjf_preemptive(self):
        ...
        while completed < n:
            while proc and proc[0].arrival <= time:
                ready_queue.append(proc.pop(0))

            if ready_queue:
                ready_queue.sort(key=lambda x: (x.remaining, x.arrival))
                if current_process is None or current_process.remaining > ready_queue[0].remaining:
                ...
                if current_process.remaining == 0:
                    ...
                    turn = time - current_process.arrival
                    wait = turn - current_process.burst
                    total_wait += wait
                    total_turn += turn
                    result += f"Process {current_process.pid}: Waiting Time={wait}, Turnaround Time={turn}\n"
                    current_process = None
            else:
                time += 1

        result += f"\nAverage Waiting Time: {total_wait/n:.2f}\n"
        result += f"Average Turnaround Time: {total_turn/n:.2f}\n"
        return result
